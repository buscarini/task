...
class Alternative: XCTestCase {
    ...
    func testAlternative1() {
		...
		(or(Task<Int>.rejected(exampleError()), Task.of(22)))
			.fork({ error in
				XCTFail()
			},
			{ value in
	...
	func testAlternative2() {
		...
		(or(Task.of(22), Task<Int>.rejected(exampleError())))
			.fork({ error in
				XCTFail()
			},
			{ value in
	...
	func testAlternativeFail() {
		...
		(or(Task<Int>.rejected(exampleError()), Task<Int>.rejected(exampleError())))
			.fork({ error in
				expectation.fulfill()
			},
			{ value in
	...
	func testAlternativeOp() {
		...
		(Task<Int>.rejected(exampleError()) <|> Task.of(22))
			.fork({ error in
				XCTFail()
			},
			{ value in
	...
	func testAlternativeOp3() {
		...
		(Task<Int>.rejected(exampleError()) <|> Task<Int>.rejected(exampleError()) <|> Task.of(22))
			.fork({ error in
				XCTFail()
			},
			{ value in
